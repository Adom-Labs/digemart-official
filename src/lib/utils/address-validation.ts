...
export function validateAddress(
  ...
  if (validatePostalCode && address.postalCode && address.country) {
    ...
    if (pattern && !pattern.test(address.postalCode)) {
      ...
      if (suggestCorrections) {
        suggestions.push(getPostalCodeSuggestion(address.country));
      }
    }
  }

  // Phone number validation
  if (validatePhone && address.phone && address.country) {
    const pattern = PHONE_PATTERNS[address.country];
    if (pattern && !pattern.test(address.phone)) {
      if (strictValidation) {
        errors.push(`Invalid phone number format for ${address.country}`);
      } else {
        warnings.push(
          `Phone number format may be incorrect for ${address.country}`
        );
      }
    }
  }
  ...
  if (address.address) {
    const addressValidation = validateAddressFormat(address.address);
    if (!addressValidation.isValid) {
      warnings.push(...addressValidation.warnings);
      if (suggestCorrections && addressValidation.suggestions) {
